...
class daily_stock_report(models.TransientModel):
    ...
    location_id = fields.Many2one('stock.location', 'Location', default=8)
    product_ids = fields.Many2many('product.product', 'product_rel_report_wiz', 'wiz_id',
                                   'product_id', 'Products')
    product_id = fields.Many2one('product.product', 'Product', default=6810)
    show_valuation = fields.Boolean('Valuation', default=False,
                                    help='Show valuation of stock?')
    skip_zero_stock = fields.Boolean('Skip Zero Stock?', default=True,
                                    help='Skip locations / products who has 0 stock?')
    all_locations = fields.Boolean('All Locations?', default=False, help='Select to ')
    report_by = fields.Selection([('detailed_report', 'Detailed Report'),
                                  ('category_summarised', 'Category/Product Wise Report'),
                                  # ('location_wise', 'Location Wise Report'),
                                  # ('all', 'Summarised Report')
    ...
    def get_product_available(self, product, from_date=False, to_date=False, location=False,
        ...
        print("Prevous=== date===",from_date,to_date),
        ...
        self._cr.execute(
            ...
            and product_id IN %s and move.state = 'done' 
            and move.picking_id is null
            and (move.is_inventory is null or move.is_inventory = False)
            '''
            + (date_str and 'and ' + date_str + ' ' or '') + \
            '''
            group by product_id, product_uom''', tuple(where))
        ...
        print("results_production_in 555", results_production_in)
        print("Where========", where)
        ...
        for quantity, prod_id, prod_uom in results_production_out:
            production -= quantity
        print("1700====", incoming_purchases,incoming_returns,outgoing,internal,adjustment,production)
        return {
    ...
    def get_opening_stock(self, moves_domain, all_locations, previous_date):
        ...
        for move in stocks:
            ...
            if move.location_id.id not in all_locations.ids and move.location_dest_id.id in \
                    all_locations.ids and move.picking_id and not move.is_inventory and \
                ...
                print('Purchase++++++++++')
                    ...
                    all_locations.ids and move.picking_id and not move.is_ventory and \
            ...
            # Adjustement
                    ...
                    all_locations.ids and not move.picking_id and move.is_inventory:
                col_count = 9
                qty_on_hand += move.quantity_done
                # print('Adjustment IN ++++++++++')
            elif move.location_id.id in all_locations.ids and move.location_dest_id.id not in \
            ...
            # Production
                    ...
                    all_locations.ids and not move.picking_id and not move.is_inventory:
                col_count = 10
                qty_on_hand += move.quantity_done
                # print('Production IN ++++++++++')
            elif move.location_id.id in all_locations.ids and move.location_dest_id.id not in \
    ...
    def detailed_movement_report(self):
        ...
        worksheet.set_column(10, 12, 17)
        worksheet.set_column(11, 13, 12)
        worksheet.set_column(12, 14, 12)
        worksheet.set_column(13, 15, 15)
        worksheet.set_row(0, 25)
        worksheet.set_row(1, 25)
        worksheet.set_row(2, 25)
        row, col = 0, 0
        worksheet.merge_range(row, col + 1, row, col + 5, self.company_id and self.company_id.name or '', style_header)
        row += 1
        worksheet.merge_range(row, col + 1, row, col + 5, "Detailed Report", style_header)
        row += 1
        worksheet.merge_range(row, col + 1, row, col + 5, self.warehouse_id and self.warehouse_id.name or '', style_header)
        row += 1
        # worksheet.write(row, col, 'Company', style_header2)
        ...
        worksheet.write(row, col + 2, '', style_header2)
        # warehouse_name = ''
        # if self.warehouse_id:
        #     warehouse_name = warehouse_name + self.warehouse_id.name + ', '
        # else:
        ...
        row += 2
        col = 0
        worksheet.write(row, col, 'WDO', style_header2)
        worksheet.write(row, col + 1, 'Ref', style_header2)
        worksheet.write(row, col + 2, 'Date', style_header2)
        worksheet.write(row, col + 3, 'Name', style_header2)
        worksheet.write(row, col + 4, 'Contact', style_header2)
        worksheet.write(row, col + 5, 'Opening', style_header2)
        worksheet.write(row, col + 6, 'Purchase', style_header2)
        worksheet.write(row, col + 7, 'Purchase Return', style_header2)
        worksheet.write(row, col + 8, 'Sales', style_header2)
        worksheet.write(row, col + 9, 'Sales Return', style_header2)
        worksheet.write(row, col + 10, 'Internal Transfer', style_header2)
        worksheet.write(row, col + 11, 'Adjustment', style_header2)
        worksheet.write(row, col + 12, 'Production/Others', style_header2)
        worksheet.write(row, col + 13, 'Value', style_header2)
        worksheet.write(row, col + 14, 'On Hand', style_header2)
        worksheet.write(row, col + 15, 'Average Cost', style_header2)
        if self.show_valuation:
        ...
        if self.from_date:
            ...
            # print("localise date=123=== ", previous_date)
            my_datetime = datetime(self.from_date.year, self.from_date.month, self.from_date.day)
            print('my_datetime', my_datetime)
            previous_date = datetime.strptime(
                str(my_datetime), DEFAULT_SERVER_DATETIME_FORMAT) - timedelta(seconds=1)
            print("d1111 5555=====", previous_date)
            ...
            if self.location_id:
                ...
                location = self.env['stock.location'].search([
                    ('usage', '=', 'internal'), '|', ('company_id', '=', self.company_id.id),
                    ('company_id', '=', False)], order='level asc')
            opening_dict = self.get_product_available(self.product_id, to_date=previous_date, location=location)

            qty_available = opening_dict.get('balance', 0.0)
            # qty_available = 0
            print("qty Available====", qty_available, opening_dict)
            # print('qty_available..2.', qty_available)
            qty_on_hand += qty_available
            worksheet.write(row, col, '', style_data3)
            worksheet.write(row, col + 1, '', style_data3)
            worksheet.write(row, col + 2, self.from_date or ' ', date_format)
            worksheet.write(row, col + 3, 'Opening', style_data3)
            worksheet.write(row, col + 4, '', style_data3)
            worksheet.write(row, col + 5, qty_available or 0, style_data3)
            ...
            worksheet.write(row, col + 13, qty_available * self.product_id.standard_price or 0, style_data3)
            worksheet.write(row, col + 14, qty_available or 0, style_data3)
            worksheet.write(row, col + 15, self.product_id.standard_price or 0, style_data3)
            row += 1
        ...
        for move in stocks:
            ...
            if move.location_id.id not in all_locations.ids and move.location_dest_id.id not in all_locations.ids:
                # print('skiping Location..id, reference...', move.id, move.reference)
                continue
            # worksheet.write(row, col, move.picking_id.x_studio_wdo or '', style_data3)
            worksheet.write(row, col, '', style_data3)
            worksheet.write(row, col + 1, move.reference, style_data3)
            worksheet.write(row, col + 2, move.date or ' ', date_format)
            worksheet.write(row, col + 3, move.name, style_data3)
            worksheet.write(row, col + 4, move.picking_id.partner_id.name or '', style_data3)
            ...
            worksheet.write(row, col + 14, '', style_data3)
            worksheet.write(row, col + 15, '', style_data3)
            col_count = 5
            quantity = move.quantity_done
            # print('move.reference quantity....', move.id, move.reference, quantity)
            # print('move.location....', move.location_id, move.location_dest_id)
            ...
            worksheet.write(row, col + 13, qty_on_hand * self.product_id.standard_price or 0, style_data3)
            worksheet.write(row, col + 14, qty_on_hand or 0, style_data3)
            worksheet.write(row, col + 15, self.product_id.standard_price or 0, style_data3)
            row += 1
